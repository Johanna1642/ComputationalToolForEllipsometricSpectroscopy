...
class DispersionModels(object):
    ...
    def get_lorenz_model(self):
        ...
        * Return epsilon_1 and epsilon_2
        """
        a = (self.electron_charge**2 * (self.parameters[0]/self.h)) / (
            self.permitivitty_vacuum * self.electron_mass
        )
        b = (self.parameters[1]/self.h) ** 2 - (self.parameters[2]/self.h) ** 2
        c = (self.parameters[3]/self.h) * (self.parameters[2]/self.h)
    ...
    def get_drude_model(self):
        ...
        * Return epsilon_1 and epsilon_2
        """
        a = (self.parameters[0]/self.h) **2
        b = a * (self.parameters[3]/self.h)
        c = (self.parameters[2]/self.h)**2 + (self.parameters[3]/self.h)**2
        
        epsilon_1 = self.parameters[1] - ((a) / (c))
        epsilon_2 = (b) / ((self.parameters[2]/self.h) * c)
        n_k = get_nk_from_dielectric_fuction(epsilon_1, epsilon_2)
        return n_k
